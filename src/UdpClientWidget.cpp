...
UdpClientWidget::UdpClientWidget( SocketConfig * config, QWidget * parent )
...
void UdpClientWidget::connected()
{
	auto socket = property( "socket" ).value<QUdpSocket *>();
	if( socket == nullptr )
	{
...
void UdpClientWidget::disconnected()
{
	auto socket = property( "socket" ).value<QUdpSocket *>();
	if( socket != nullptr )
	{
...
void UdpClientWidget::on_pushButton_connect_clicked()
{
	...
	if( socket != nullptr )
	{
		setProperty( "socket", QVariant::fromValue( ( QUdpSocket *)nullptr ) );
	...
	if( ui.pushButton_connect->text() == QString::fromLocal8Bit( "连接" ) )
	{
		...
		connect( socket, &QUdpSocket::readyRead, [this, socket]()
		{
			if( socket->hasPendingDatagrams() )
			{
				...
				auto bytes = QByteArray::fromRawData( buf, socket->readDatagram( buf, 1024 ) );
		...
		connect( socket, ( void( QUdpSocket:: * )( QAbstractSocket::SocketError ) ) & QUdpSocket::error, [this, socket]()
		{
			switch( socket->state() )
			{
		...
		if( socket->bind() )
		{
			...
			if( ui.checkBox_auto_send->isChecked() )
			{
				if( ui.lineEdit_send_interval->text().toInt() == 0 )
				{
					...
					socket->writeDatagram( bytes, QHostAddress( ui.lineEdit_target_ip->text() ), ui.lineEdit_target_port->text().toUShort() );
...
void UdpClientWidget::on_pushButton_clear_count_clicked()
{
	ui.label_send_count->setText( "0" );
	ui.label_recv_count->setText( "0" );
}

void UdpClientWidget::on_pushButton_send_send_clicked()
{
	auto socket = property( "socket" ).value<QUdpSocket *>();
	if( socket != nullptr )
	{
...
void UdpClientWidget::on_pushButton_send_clear_clicked()
{
	ui.textEdit_send->clear();
}

void UdpClientWidget::on_pushButton_recv_save_clicked()
{
...
void UdpClientWidget::on_pushButton_recv_clear_clicked()
{
	ui.textEdit_recv->clear();
}

void UdpClientWidget::on_checkBox_auto_send_clicked( bool clicked )
{
	config()->auto_send = clicked;
	emit socketChanged( config() );
}

void UdpClientWidget::on_checkBox_auto_connect_clicked( bool clicked )
{
	config()->auto_connect = clicked;
	emit socketChanged( config() );
}

void UdpClientWidget::on_lineEdit_name_textChanged( const QString & text )
{
	config()->name = text;
	emit socketChanged( config() );
}

void UdpClientWidget::on_lineEdit_target_ip_textChanged( const QString & text )
{
	config()->host = text;
	emit socketChanged( config() );
}

void UdpClientWidget::on_lineEdit_target_port_textChanged( const QString & text )
{
	config()->port = text.toUShort();
	emit socketChanged( config() );
}

void UdpClientWidget::on_lineEdit_send_interval_interval_textChanged( const QString & text )
{
